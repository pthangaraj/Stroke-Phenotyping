...
#train_mrns=np.load('train_mrns_v5_subwDemog_CID_1to1_dera'+case+control+'.npy')
...
c = db.cursor()
SQL ='''select person_id, 0 as label from user_pt2281.rand_person_id_merge where person_id not in %s union select person_id, 1 as label from user_pt2281.STROKE_GOLD_STANDARD_test_merge where person_id not in %s''' %(str(tuple(train_mrns)),str(tuple(train_mrns)))
#SQL ='''select person_id, 0 as label from clinical_cumc_v5.condition_occurrence where person_id not in %s''' %(str(tuple(train_mrns)))
c.execute(SQL)
results = c.fetchall()
...
#############EHR
filename_mrns='mrns_testwCID_1to1_cond_dera_sou_merge_raceeth_mergeset_noctrlicd_new_fixcv' + case+control+'.npy'
np.save(filename_mrns,mrns)
filename_labels='labels_testwCID_1to1_cond_dera_sou_merge_raceeth_mergeset_noctrlicd_new_fixcv' + case + control +'.npy'
...
filename_mtest='matrix_testwCID_1to1_cond_dera_sou_merge_raceeth_binage_mergeset_noctrlicd_new_fixcv' + case + control + '.npz'
